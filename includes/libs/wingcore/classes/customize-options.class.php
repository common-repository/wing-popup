 ...
 *
 */
if ( ! class_exists( 'WDPOP_CORE_Customize_Options' ) ) {
  class WDPOP_CORE_Customize_Options extends WDPOP_Abstract {
    ...
    public function add_customize_save_after( $wp_customize ) {
      do_action( "wdpop_core_{$this->unique}_save_before", $this->get_options(), $this, $wp_customize );
      do_action( "wdpop_core_{$this->unique}_saved", $this->get_options(), $this, $wp_customize );
      do_action( "wdpop_core_{$this->unique}_save_after", $this->get_options(), $this, $wp_customize );
    }
    ...
    public function save_defaults() {
      ...
      if ( $this->args['save_defaults'] && empty( $this->args['show_in_customizer'] ) && empty( $tmp_options ) ) {
         ...
         */
          update_option( $this->unique, $this->options ); // @codingStandardsIgnoreLine
        }
    ...
    public function add_customize_options( $wp_customize ) {
      ...
      if ( ! class_exists( 'WDPOP_Customize_Panel' ) ) {
        WDPOP_CORE::include_plugin_file( 'functions/customize.php'  );
      }
      ...
      foreach ( $this->pre_sections as $section ) {
        ...
        if ( ! empty( $section['subs'] ) ) {
          ...
          $wp_customize->add_panel( new WDPOP_Customize_Panel( $wp_customize, $panel_id, array(
    ...
    public function add_section( $wp_customize, $section_id, $section_args, $panel_id ) {
      ...
      if ( ! empty( $section_args['assign'] ) ) {
        ...
        $wp_customize->add_section( new WDPOP_Customize_Section( $wp_customize, $section_id, array(
      ...
      if ( ! empty( $section_args['fields'] ) ) {
        ...
        foreach ( $section_args['fields'] as $field ) {
          ...
          $wp_customize->add_control( new WDPOP_Customize_Control( $wp_customize, $setting_id,
