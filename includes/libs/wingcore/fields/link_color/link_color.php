 ...
 *
 */
if ( ! class_exists( 'WDPOP_CORE_Field_link_color' ) ) {
  class WDPOP_CORE_Field_link_color extends WDPOP_Fields {
    ...
    public function render() {
      ...
      $color_props = array(
        'color'    => esc_html__( 'Normal', 'wing-popup' ),
        'hover'    => esc_html__( 'Hover', 'wing-popup' ),
        'active'   => esc_html__( 'Active', 'wing-popup' ),
        'visited'  => esc_html__( 'Visited', 'wing-popup' ),
        'focus'    => esc_html__( 'Focus', 'wing-popup' )
      );
      ...
      foreach ( $color_props as $color_prop_key => $color_prop_value ) {
        ...
        if ( ! empty( $args[$color_prop_key] ) ) {
          ...
          echo '<div class="wdpop_core--left wdpop_core-field-color">';
          echo '<div class="wdpop_core--title">'. esc_attr( $color_prop_value ) .'</div>';
          echo '<input type="text" name="'. esc_attr( $this->field_name( '['. $color_prop_key .']' ) ) .'" value="'. esc_attr( $value[$color_prop_key] ) .'" class="wdpop_core-color"'. wp_kses_post($default_attr . $this->field_attributes() ).'/>';
          echo '</div>';
